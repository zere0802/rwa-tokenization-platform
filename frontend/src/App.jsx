...
import { CONTRACTS, tokenABI, vaultABI, governorABI } from "./contracts";
...
function App() {
  ...
  const wrongNetwork = isConnected && chainId !== 84532 && chainId !== 31337;
  ...
  const { data: delegates, refetch: refetchDelegates } = useReadContract({
    address: tokenAddress,
    abi: tokenABI,
  ...
  async function depositToVault() {
    try {
      ...
      setTxMessage(err.shortMessage || "Transaction failed");

      if (err.message?.includes("User rejected")) {
        setTxError(true);

        setTxMessage(err.shortMessage || "Transaction rejected by user");
      } else {
        setTxError(true);

        setTxMessage(err.shortMessage || "Transaction failed or reverted");
      }
    }
  }
  ...
  async function approveVault() {
    try {
      ...
      setTxMessage("Vault approved successfully");
    } catch (err) {
      setTxError(true);

      setTxMessage(err.shortMessage || "Approve transaction failed");
    }
  }
  ...
  async function delegateVotes() {
    try {
      ...
      setTxMessage("Votes delegated successfully");

      window.location.reload();
    } catch (err) {
      setTxError(true);

      setTxMessage(err.shortMessage || "Delegation failed");
    }
  }
  ...
  async function createProposal() {
    try {
      const calldata = encodeFunctionData({
        abi: tokenABI,
        functionName: "approve",
        args: [vaultAddress, parseEther("1")],
      });
      ...
      const hash = await writeContractAsync({
        ...
        args: [[tokenAddress], [0], [calldata], proposalTitle],
      });
      ...
      setTxMessage(err.shortMessage || "Proposal creation failed");
    }
  }
  ...
  async function vote(support) {
    try {
      ...
      setTxMessage(err.shortMessage || "Vote transaction failed");
    }
  }
    ...
    async function fetchData() {
      if (chainId === 31337) {
        ...
        try {
          ...
          const parsedProposals = proposalLogs
            .map((log) => ({
  ...
  useEffect(() => {
    if (!publicClient || !governorAddress || subgraphProposals.length === 0)
      return;
    ...
    <div className="app">
      <div className="hero">
        {wrongNetwork && (
          ...
          >
            Wrong network detected. Please switch to Base Sepolia or Localhost.
          </div>
        )}
        {txMessage && (
          <div
            style={{
              ...
              border: txError ? "2px solid #ffb7b7" : "2px solid #bdf5c4",
          ...
          >
            <div>
              ...
              >
                {txError ? "Transaction Failed" : "Transaction Successful"}
              </div>
        ...
        <div className="hero-grid">
          <div>
            <p className="hero-subtitle">Tokenize • Govern • Earn</p>
            ...
            <p className="hero-description">
              A decentralized platform for tokenized real-world assets, vault
              deposits, DAO governance, and onchain voting.
            </p>
          </div>
          ...
          <div className="wallet-card-wrapper">
            <div className="wallet-card">
              <div className="wallet-header">
                <div>
                  <p className="wallet-title">Connected Wallet</p>
              ...
              <div className="stats">
                ...
                <div className="stat-card votes">
                  <p className="stat-label">Voting Power</p>
                ...
                <div className="stat-card delegate">
                  <p className="stat-label">Delegate Address</p>
                  ...
                  <button onClick={delegateVotes} className="button check-btn">
                    Delegate Votes
                  </button>
                </div>
              </div>
      ...
      <div className="section">
        <div className="section-card deposit-card">
          <h2 className="section-title">Vault Deposit</h2>
          ...
          <div className="deposit-row">
            ...
            <button onClick={depositToVault} className="button deposit-btn">
              Deposit
            </button>
          </div>
        </div>
      ...
      <div className="section">
        <div className="section-card governance-card">
          <div className="gov-header">
            <div>
              <p className="hero-subtitle">DAO Governance</p>

              <h2 className="section-title">Governance Portal</h2>
            </div>
            <input
              type="text"
            ...
            <button onClick={createProposal} className="button proposal-btn">
              Create Proposal
            </button>
          </div>
          ...
          >
            ...
            >
              <p className="stat-label">Indexed Governance Proposals</p>

              {subgraphProposals.length === 0 ? (
                <p>
                  No governance proposals indexed yet. Create your first DAO
                  proposal.
                </p>
              ) : (
                subgraphProposals.map((proposal, index) => (
                  ...
                  >
                    ...
                    >
                      Proposal #{proposal.proposalId.slice(0, 12)}
                      ...
                    </p>
                    ...
                    <a
                      href={
                        chainId === 31337
                          ? "#"
                          : `https://sepolia.basescan.org/address/${governorAddress}`
                      }
                      target="_blank"
                      rel="noreferrer"
                      style={{
            ...
            >
              <p className="stat-label">Recent Governance Votes</p>

              {subgraphVotes.length === 0 ? (
                <p>No votes indexed yet</p>
              ) : (
                subgraphVotes.map((vote, index) => {
                  const proposal = subgraphProposals.find(
                    (p) => p.proposalId === vote.proposalId,
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {proposal ? proposal.description : "Vote Activity"}
                      </p>
            ...
            >
              <p className="stat-label">Proposal ID</p>
              ...
              <button onClick={checkProposalState} className="button check-btn">
              ...
              <div className="state-box">
                ...
                <h3 className="state-value">{proposalState || "—"}</h3>
              </div>
            </div>
            ...
            >
              <p className="stat-label">Cast your governance vote</p>

              <div className="vote-buttons">
                <button onClick={() => vote(1)} className="button vote-for">
                  Vote FOR
                </button>

                <button onClick={() => vote(0)} className="button vote-against">
                  Vote AGAINST
                </button>
              </div>
